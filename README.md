...
In order to easily run the code, you must have installed the Keras==2.1.5 framework with TensorFlow==1.5.0 backend. The Darknet framework is self-contained in the "darknet" folder and must be compiled before running the tests. To build Darknet just type "make" in "darknet" folder:

```shellscript
$ cd darknet && make
```

**The current version was tested in an Ubuntu 18.04.2 machine, with Keras 2.1.5, TensorFlow 1.5.0, OpenCV 4.0.0, NumPy 1.16.5 and Python 2.7.**
